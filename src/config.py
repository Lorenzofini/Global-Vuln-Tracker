...
def load_environment_variables(env_path: str = ".env") -> Dict[str, str]:
    """Carica le variabili d'ambiente e le restituisce in un dizionario."""
    ...
    for var in required_vars:
        value = os.getenv(var)
        if not value:
            raise ValueError(f"Variabile d'ambiente richiesta '{var}' non impostata nel file .env o nell'ambiente.")
        env_vars[var] = value
